...
impl PostgresAuthWebRuntime {
    pub(crate) async fn execute_authenticated_credential_regeneration_planning_from_headers(
        &self,
        headers: &HeaderMap,
        request: PlanAuthenticatedCredentialRegenerationInput,
    ) -> Result<AuthWebRuntimeExecution, AuthPostgresWebRuntimeExecutionError> {
        let now = request.now;
        let regeneration_policy = &self
            .runtime
            .config()
        ...
        if let Some(outcome) = lifecycle_step_up_freshness_outcome(
            now,
            session,
            regeneration_policy.authenticated_planning_step_up_freshness,
        ) {
        ...
        let pending_action = match pending_credential_lifecycle_action_schedule_from_policy(
            now,
            regeneration_policy.delayed_action_timing,
        ) {
        ...
        let command = Command::PlanCredentialRegeneration(PlanCredentialRegeneration {
            now,
            lifecycle_context,
            independent_evidence_required: regeneration_policy.independent_evidence_requirement,
            pending_action,
        });
        ...
        self.commit_runtime_owned_prepared_command_inside_open_transaction(
            "auth_core.runtime.authenticated_credential_regeneration_planning",
            now,
            tx,
            prepared,
            presented_cookie_secrets,
    ...
    pub(crate) async fn execute_authenticated_credential_rotation_from_headers(
        &self,
        headers: &HeaderMap,
        request: ExecuteAuthenticatedCredentialRotationInput,
    ) -> Result<AuthWebRuntimeExecution, AuthPostgresWebRuntimeExecutionError> {
        let now = request.now;
        let rotation_policy = &self
            .runtime
            .config()
            .credential_lifecycle_policy
            .credential_rotation;
        ...
        if let Some(outcome) = lifecycle_step_up_freshness_outcome(
            now,
            session,
            rotation_policy.authenticated_execution_step_up_freshness,
        ) {
            return rollback_and_return_outcome(tx, outcome).await;
        }
        let evidence_sources = [LifecycleAuthoritySource::AuthenticatedSession(
        ...
        let (method_commit_work, post_commit_method_response_material) = match self
            .method_registry()
        {
            Ok(registry) => {
                match registry
                    .build_credential_lifecycle_commit_work(
                        ...
                        CredentialLifecycleMethodWorkBuildRequest {
                            ...
                            authority: CredentialLifecycleMethodWorkAuthority::ImmediateRotation {
                    ...
                    .map_err(AuthPostgresWebRuntimeExecutionError::method_build)
                {
                    ...
                    Err(error) => {
                        return Err(rollback_after_runtime_error(
                            "auth_core.runtime.build_credential_rotation_work",
                            tx,
                            error,
                        )
                        .await);
        ...
        let command = Command::ExecuteCredentialRotation(ExecuteCredentialRotation {
            now,
            execution_authority: CredentialRotationExecutionAuthority {
                lifecycle_context,
                independent_evidence_required: rotation_policy.independent_evidence_requirement,
            },
            method_commit_work,
        });
        let prepared = match PreparedCommandExecution::prepare(
        ...
        let mut execution = self
            .commit_runtime_owned_prepared_command_inside_open_transaction(
                "auth_core.runtime.authenticated_credential_rotation",
                now,
                tx,
                prepared,
                presented_cookie_secrets,
    ...
    pub(crate) async fn execute_authenticated_credential_regeneration_from_headers(
        &self,
        headers: &HeaderMap,
        request: ExecuteAuthenticatedCredentialRegenerationInput,
    ) -> Result<AuthWebRuntimeExecution, AuthPostgresWebRuntimeExecutionError> {
        let now = request.now;
        let regeneration_policy = &self
            .runtime
        ...
        if let Some(outcome) = lifecycle_step_up_freshness_outcome(
            now,
            session,
            regeneration_policy.authenticated_execution_step_up_freshness,
        ) {
            return rollback_and_return_outcome(tx, outcome).await;
        }
        let evidence_sources = [LifecycleAuthoritySource::AuthenticatedSession(
            ...
            match self.method_registry() {
                Ok(registry) => {
                        ...
                        .map_err(AuthPostgresWebRuntimeExecutionError::method_build)
                    {
                        ...
                        Err(error) => {
                            return Err(rollback_after_runtime_error(
                                "auth_core.runtime.build_credential_regeneration_work",
                                tx,
                                error,
                            )
                            .await);
        ...
        let command = Command::ExecuteCredentialRegeneration(ExecuteCredentialRegeneration {
            now,
        ...
        let mut execution = self
            .commit_runtime_owned_prepared_command_inside_open_transaction(
                "auth_core.runtime.authenticated_credential_regeneration",
                now,
                tx,
                prepared,
                presented_cookie_secrets,
