...
async fn fleet_schema_setup_and_validation_emit_expected_database_operation_shapes() {
    let Some(database_url) = test_database_url() else {
        eprintln!(
            "skipping Postgres Fleet schema operation-count test; set TEST_DSN or PARANOID_TEST_DATABASE_URL to run"
        );
        return;
    };
...
async fn fleet_schema_migration_rolls_back_when_existing_schema_is_incompatible() {
    let Some(database_url) = test_database_url() else {
        eprintln!(
            "skipping Postgres Fleet schema rollback test; set TEST_DSN or PARANOID_TEST_DATABASE_URL to run"
        );
        return;
    };
