...
fn fleet_constructor_api_surface_compiles_through_namespaced_exports() {
    ...
    let mutex: crate::fleet::Mutex = store
        .new_mutex(
            MutexKey::new("api-mutex").expect("mutex key"),
            claim_duration,
        )
        .expect("new mutex");
    let counter: crate::fleet::Counter = store
        .new_counter(CounterKey::new("api-counter").expect("counter key"))
        .expect("new counter");
    let cache: crate::fleet::CoalescingCache<String> = store
        ...
        .expect("new cache");
    let topic: crate::fleet::Topic<String> = store
    ...
    let cron: crate::fleet::Cron = store
        ...
        .expect("new cron");
    let semaphore: crate::fleet::Semaphore = store
        .new_semaphore(
    ...
    let throttler: crate::fleet::Throttler = store
    ...
    let rate_limiter: crate::fleet::RateLimiter = store
        .new_rate_limiter(
    ...
    let circuit_breaker: crate::fleet::CircuitBreaker = store
        .new_circuit_breaker(
    ...
    let once: crate::fleet::Once = store
        .new_once(OnceKey::new("api-once").expect("once key"), claim_duration)
        .expect("new once");

    let _: crate::fleet::manual::MutexManualRenewalProtocol<'_> =
        mutex.begin_manual_renewal_lifecycle();
    let _: crate::fleet::manual::SemaphoreManualClaimProtocol<'_> =
        semaphore.begin_manual_claim_lifecycle();
    let _: crate::fleet::manual::ThrottlerManualPermitProtocol<'_> =
        throttler.begin_manual_permit_lifecycle();
    let _: crate::fleet::manual::RateLimiterManualPermitProtocol<'_> =
        rate_limiter.begin_manual_permit_lifecycle();
    let _: crate::fleet::manual::CircuitBreakerManualPermitProtocol<'_> =
        circuit_breaker.begin_manual_permit_lifecycle();
    let _: crate::fleet::manual::OnceManualRunProtocol<'_> = once.begin_manual_run_lifecycle();
...
fn fleet_table_names_must_not_overlap() {
    ...
    assert!(matches!(
        ...
        Err(crate::fleet::Error::TableNamesMustBeDistinct)
    ));
    ...
    let ambiguous_state_table =
        crate::db::PgQualifiedTableName::unqualified("__paranoid_same_fleet_table").expect("table");
...
fn fleet_constructor_options_apply_defaults_without_database_work() {
    ...
    assert_eq!(
        ...
        crate::fleet::DEFAULT_CRON_CLAIM_DURATION
    );
    ...
    assert_eq!(
        ...
        crate::fleet::DEFAULT_COALESCING_CACHE_LOCK_WAIT_TIMEOUT
    );
    assert_eq!(cache.compute_timeout(), None);

    let _default_mutex_guard_config = MutexGuardConfig::default();
    let _: Duration = crate::fleet::DEFAULT_MUTEX_MAX_ACQUIRE_RETRY_INTERVAL;
}
