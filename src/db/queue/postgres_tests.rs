use crate::db::fleet as db_fleet;
...
use crate::db::queue::StoreConfig;
use crate::db::{PgIdentifier, PgQualifiedTableName, PoolConfig, WritePool};
use crate::fleet;
use crate::fleet::CronKey;
use crate::queue::{
    ...
    Store, TaskError, TaskRegistry, WorkerConfig, WorkerDefaultJobTimeout, WorkerMaintenanceConfig,
    WorkerOwnerId,
};
...
#[path = "postgres_tests/dead_letter_cancellation.rs"]
mod dead_letter_cancellation;
#[path = "postgres_tests/enqueue_and_observability.rs"]
mod enqueue_and_observability;
#[path = "postgres_tests/locking.rs"]
mod locking;
#[path = "postgres_tests/maintenance_and_listing.rs"]
mod maintenance_and_listing;
#[path = "postgres_tests/maintenance_locking.rs"]
mod maintenance_locking;
#[path = "postgres_tests/maintenance_stage_errors.rs"]
mod maintenance_stage_errors;
#[path = "postgres_tests/retry_and_dead_letter.rs"]
mod retry_and_dead_letter;
#[path = "postgres_tests/schema.rs"]
mod schema;
#[path = "postgres_tests/schema_operation_contracts.rs"]
mod schema_operation_contracts;
#[path = "postgres_tests/schema_validation.rs"]
mod schema_validation;
#[path = "postgres_tests/transactions.rs"]
mod transactions;
#[path = "postgres_tests/worker_cleanup_failures.rs"]
mod worker_cleanup_failures;
#[path = "postgres_tests/worker_fleet_maintenance.rs"]
mod worker_fleet_maintenance;
#[path = "postgres_tests/worker_locking.rs"]
mod worker_locking;
#[path = "postgres_tests/worker_loop.rs"]
mod worker_loop;
#[path = "postgres_tests/worker_loop_shutdown.rs"]
mod worker_loop_shutdown;
#[path = "postgres_tests/worker_once.rs"]
mod worker_once;
#[path = "postgres_tests/worker_once_claims.rs"]
mod worker_once_claims;
#[path = "postgres_tests/worker_once_support.rs"]
...
impl TestDatabase {
    async fn connect() -> Self {
        let database_url = queue_test_database_url();
...
fn unique_fleet_test_config() -> db_fleet::StoreConfig {
    let suffix = crate::queue::JobId::new()
        .expect("new job id")
        .to_string()
        .replace('-', "_");
    db_fleet::StoreConfig::new(
...
async fn drop_fleet_test_tables(pool: &PgPool, config: &db_fleet::StoreConfig) {
...
async fn lock_dead_letter_job_row<'a>(
    test_database: &'a TestDatabase,
    dead_letter_job_id: crate::queue::JobId,
) -> sqlx::Transaction<'a, sqlx::Postgres> {
    let mut tx = test_database
        .sqlx_pool
        .begin()
...
fn assert_worker_owner_id_was_derived_from_worker_name(
    ...
    assert_eq!(
        worker_owner_id.len(),
        worker_name.len() + 1 + crate::id::SORTABLE_ID_TEXT_LEN
    );
...
) -> Result<Vec<crate::queue::Job>, Error> {
