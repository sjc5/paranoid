...
async fn queue_dead_letter_batch_moves_only_failed_rows_and_rejects_oversized_inputs() {
    ...
    let missing = paranoid::queue::JobId::new().expect("missing id");
    ...
    let oversized_ids = (0..=paranoid::queue::MAX_DEAD_LETTER_MOVE_BATCH_SIZE)
        .map(|_| paranoid::queue::JobId::new().expect("oversized id"))
        .collect::<Vec<_>>();
...
async fn queue_job_status_and_single_dead_letter_miss_paths_are_exact() {
    ...
    let missing = paranoid::queue::JobId::new().expect("missing job id");
